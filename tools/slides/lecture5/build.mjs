...
const BUILD=path.join(ROOT,'.build/slides/lecture5');
...
const SKILL='/home/william/.codex/plugins/cache/openai-primary-runtime/presentations/26.909.12148/skills/presentations';
...
const C=JSON.parse(await fs.readFile(path.join(ROOT,'tools/slides/lecture5/content.json'),'utf8'));
...
while(p.slides.items.length>C.length)p.slides.items.at(-1).delete();
...
async function media(s,f,x,y,w,h){const b=await fs.readFile(path.join(BUILD,'assets',f));s.images.add({blob:new Uint8Array(b),contentType:f.endsWith('.gif')?'image/gif':'image/png',alt:`Teaching simulator output: ${f}`,fit:'contain',position:{left:x,top:y,width:w,height:h}})}
...
for(let i=0;i<C.length;i++){
 ...
 if(i===0){text(s,'LECTURE 5',65,95,800,40,23,'#FFFFFF',true);text(s,'Localization',65,160,810,80,52,'#FFFFFF',true);text(s,'for autonomous driving',65,245,810,55,37,'#FFFFFF');text(s,c.body[1],65,330,810,60,26,'#FFFFFF');text(s,c.body[2],65,420,810,35,22,'#FFFFFF');text(s,'1',885,507,30,25,16,'#FFFFFF');}
 else{
 ...
 if(c.demo)link(s,'Open interactive demo',`https://croquembouche.github.io/Intro2AV-CourseContent/localization/#${c.demo}`,55,438,540);
 ...
 link(s,'Open Localization demos', 'https://croquembouche.github.io/Intro2AV-CourseContent/localization/',60,350,800);
 link(s,'Download the lecture GIFs', 'https://croquembouche.github.io/Intro2AV-CourseContent/localization/gifs/',60,400,800);
