...
export function layersFor(model, image, result) {
  ...
  if(model==='cnn') {
    ...
    return [maps('input','Input layer · 32 × 32 padded',1,32,0,[padded],'','input'),
      maps('relu1','Conv 1 + activation · 6 maps',6,28,34,d?.relu1,'input','conv'),
      maps('pool1','Max pooling 1 · 6 maps',6,14,60,d?.pool1,'relu1','pool'),
      maps('relu2','Conv 2 + activation · 16 maps',16,10,90,d?.relu2,'pool1','conv'),
  ...
  return [maps('input','Input image · 16 patches',1,28,0,[image],'','patches'),
    tokens('tokens0','Initial tokens · 17 × 48',31,d?.initial,'input'),attention(0,67),
...
export function inspectNode(S, layer, channel, index) {
  ...
  if(!R||!W) return {...record,formula:R?'Loading weights for this model…':'Waiting for a model result.'};
  ...
  if(layer.kind==='dense') {
    ...
    if(S.model==='transformer') {
      // A class logit uses all 48 normalized CLS features, not raw token values.
      const cls=source.values[0].slice(0,48),mean=sum(cls)/48;
      const variance=sum(cls.map(v=>(v-mean)**2))/48;
      record.normalized=cls.map((v,j)=>(v-mean)/Math.sqrt(variance+1e-5)*W['norm.weight'][j]+W['norm.bias'][j]);
      record.contributions=record.normalized.map((v,j)=>({label:j,value:v*weight[j]}));
      record.weighted=sum(record.contributions.map(c=>c.value))+bias;
      record.contributionLabel='Largest LayerNorm(CLS) × weight terms';
      record.formula=`Σ(normalized CLS × class weight) + ${format(bias)} = logit ${format(pre)}; softmax over 10 logits → ${format(record.value)}`;
      record.links.push({layer:'tokens2',channel:0,token:0,kind:'token',role:'normalized CLS',weight:1});
      record.connectionLabel='1 CLS token · 48 features';
      record.note='The gold dashed line groups the whole CLS token. LayerNorm changes its values before the class weights are applied. Bars show the largest eight normalized contributions.';
    }else {
      ...
      if(S.model==='mlp'&&layer.id==='hidden1'&&d.disabled_neuron===index) record.formula=`Σ(input × weight) + bias = ${format(pre)}; ReLU = ${format(Math.max(0,pre))}; disabled by model control → 0`;
  ...
  if(layer.kind==='pool') {
    ...
    record.formula=`max(${record.inputGrid.map(format).join(', ')}) = ${format(record.value)}`;
    record.note='Four local responses become one; stride = 2. Bright lines mark maxima; tied maxima are not added together.';return record;
  }
  if(layer.kind==='attention') {
    ...
    // A selected score is a Q/K comparison, not an attention-weighted value sum.
    record.links=[{layer:source.id,channel:0,token:row,kind:'token',role:'query',weight:1},
      {layer:source.id,channel:0,token:key,kind:'token',role:'key',weight:1}];
    record.query=b.q[channel][row];record.key=b.k[channel][key];record.attentionRow=a;
    record.connectionLabel='2 token dependencies · Q and K';record.weightedLabel='Scaled Q · K score';record.outputLabel='Attention weight';
    ...
    record.inputGrid=a.slice(1);record.gridSize=4;record.gridLabel='Query’s attention over patches';
    record.note=`Gold: query token → Q. Pink: key token → K. Dashed lines group whole tokens (LayerNorm then projection). Grid and bars show the full query row: CLS weight ${format(a[0])}; row sum ${format(sum(a))}.`;
  ...
  if(layer.id==='tokens0') {
    ...
    if(row===0)record.formula=d.use_positions?'Learned CLS token + its learned position vector.':'Learned CLS token; positions disabled.';
    else {
    ...
    a.forEach((w,t)=>record.links.push({layer:source.id,channel:0,kind:'token',token:t,weight:w}));
    record.targetToken=row;record.attentionRow=a;record.connectionLabel='17 token mixing relationships';
    record.subtitle=`Token ${row===0?'CLS':row} · dimension ${col}`;
    record.formula=`Residual after attention ${format(b.residual[row][col])} + feed-forward ${format(b.ffn[row][col])} = ${format(record.value)}`;
    record.note=`Gold dashed lines summarize head ${head+1}'s mixing into this whole token. They are not scalar weights into the selected cube. Its value also uses all heads, output projection, residuals and the feed-forward network.`;
    record.inputGrid=a.slice(1);record.gridSize=4;record.gridLabel='Selected head’s attention';
    record.contributions=a.map((w,t)=>({label:t===0?'CLS':t,value:w*b.v[head][t][S.dimension||0]}));record.mixed=b.mixed[head][row][S.dimension||0];
  }
  return record;
}
