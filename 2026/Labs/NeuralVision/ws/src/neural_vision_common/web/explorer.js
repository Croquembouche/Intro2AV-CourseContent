...
const state={model:'mlp',image:Array(784).fill(0),results:{},weights:{},nodes:{},head:0,dimension:0,inputChannel:0,id:null,received:-1};
...
function preview(){putPixels(padPixels(),$('downsample'))}
async function publish(){if(sending){dirtyInput=true;return}sending=true;dirtyInput=false;lastSend=performance.now();const pixels=padPixels();preview();try{const response=await api('/api/infer',{pixels});ownIds.add(response.input_id);if(ownIds.size>60)ownIds.delete(ownIds.values().next().value);pending=response.input_id;notice('');if(isBrowser)await poll();}catch(e){notice(e.message)}finally{sending=false;if(dirtyInput)setTimeout(publish,80)}}
function pointAt(e){const r=pad.getBoundingClientRect();return [(e.clientX-r.left)*280/r.width,(e.clientY-r.top)*280/r.height]}
pad.onpointerdown=e=>{drawing=true;point=pointAt(e);pad.setPointerCapture(e.pointerId);ctx.fillStyle=erasing?'black':'white';ctx.beginPath();ctx.arc(...point,erasing?17:12,0,Math.PI*2);ctx.fill();preview();if($('live').checked)publish()};
pad.onpointermove=e=>{if(!drawing)return;const p=pointAt(e);ctx.strokeStyle=erasing?'black':'white';ctx.lineWidth=erasing?34:24;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(...point);ctx.lineTo(...p);ctx.stroke();point=p;preview();if($('live').checked){dirtyInput=true;if(performance.now()-lastSend>120)publish()}};
pad.onpointerup=()=>{drawing=false;if($('live').checked)publish()};pad.onpointercancel=()=>drawing=false;
...
function drawGrid(canvas,values,n,signed=false){
 ...
 values.forEach((value,i)=>{
  ...
  if(n<=5){
   ...
   c.textAlign='center';c.textBaseline='middle';c.fillText(value.toFixed(2),(i%n+.5)*cell,(Math.floor(i/n)+.5)*cell);
  }
 });
}
function drawContributions(record){
 ...
 const max=Math.max(.0001,...record.contributions.map(d=>Math.abs(d.value))),w=(right-left)/record.contributions.length;
 c.strokeStyle='#89a9bf';c.beginPath();c.moveTo(left,baseline);c.lineTo(right,baseline);c.stroke();
 record.contributions.forEach((d,i)=>{
 ...
 c.fillText(record.mixed!==undefined?`Σ a × V[${state.dimension}] = ${format(record.mixed)}`:'Largest input × weight contributions',12*uiScale,21*uiScale);
}
function inspect(record,screen,pinned){const panel=$('inspector');if(!record){panel.hidden=true;return}panel.hidden=false;panel.classList.toggle('pinned',pinned);$('nodeTitle').textContent=record.title;$('nodeSubtitle').textContent=record.subtitle;$('nodeOutput').textContent=format(record.value);$('calculation').textContent=record.formula;$('weightedRow').hidden=record.weighted===undefined;$('weighted').textContent=record.weighted===undefined?'':format(record.weighted);$('nodeNote').textContent=record.note||'';$('connectionCount').textContent=record.links.length+' connections';$('pinHint').textContent=pinned?'Pinned · Esc to release':'Click a cube to pin';panel.dataset.layer=record.layer;panel.dataset.unit=record.index;panel.dataset.channel=record.channel;
 $('gridPair').hidden=!record.inputGrid;$('weightWrap').hidden=!record.weightGrid;$('gridPair').classList.toggle('single',!record.weightGrid);$('gridLabel').textContent=record.gridLabel||'Input values';if(record.inputGrid)drawGrid($('inputGrid'),record.inputGrid,record.gridSize,record.gridLabel!==undefined);if(record.weightGrid)drawGrid($('weightGrid'),record.weightGrid,record.gridSize,true);
...
function predictions(){const r=state.results[state.model];const rank=r?r.probabilities.map((p,i)=>({p,i})).sort((a,b)=>b.p-a.p):[];$('guess').textContent=rank.length?rank[0].i:'—';$('second').textContent=rank.length?rank[1].i:'—';$('confidence').textContent=rank.length?(rank[0].p*100).toFixed(1)+'%':'';}
async function poll(){if(polling)return;polling=true;try{const data=await api('/api/state');state.nodes=data.nodes;const count=names.filter(n=>data.nodes[n]).length;$('connection').textContent=`${isBrowser?'In your browser':'ROS 2'} · ${count}/3 models${state.results[state.model]?' · '+state.results[state.model].inference_ms.toFixed(1)+' ms':''}`;$('messageCount').textContent=isBrowser?'The hosted demo computes on your device. Launch the downloaded packages to publish and inspect real ROS messages.':`${data.received} activation messages received. Input: ${data.input_id||'none'}.`;
 const signature=data.input_id+':'+data.received;if(signature!==lastSignature){lastSignature=signature;const external=data.input_id!==state.id&&!ownIds.has(data.input_id);if(data.pixels){state.image=data.pixels.map(v=>v/255);if(external&&!drawing&&!sending)putPixels(data.pixels);putPixels(data.pixels,$('downsample'))}state.id=data.input_id;state.results=data.results;refreshGraph();predictions();syncParameters();}
...
async function chooseModel(model){state.model=model;view.clearSelection();
 const copy={mlp:['Fully connected network','784 pixels → 300 → 100 → 10 digits','Try shifting the digit. Which connections receive different input?'],cnn:['Convolutional network','Shared filters → feature maps → pooling → digit scores','Inspect a feature-map cell. Watch the same filter work across the image.'],transformer:['Vision transformer','16 image patches + CLS → 2 attention blocks → digit scores','Select an attention cell. Which patches contribute to the selected query?']}[model];
 $('modelTitle').textContent=copy[0];$('modelStructure').textContent=copy[1];$('teachingPrompt').textContent=copy[2];document.querySelectorAll('[data-model]').forEach(b=>{const selected=b.dataset.model===model;b.classList.toggle('active',selected);b.setAttribute('aria-pressed',selected)});$('transformerControls').hidden=model!=='transformer';$('echoCommand').textContent=`ros2 topic echo --once /neural_demo/${model}/prediction`;if(!state.weights[model])state.weights[model]=await api('/api/weights/'+model);if(state.model!==model)return;refreshGraph();predictions();parameters();syncParameters();const url=new URL(location.href);url.searchParams.set('model',model);history.replaceState(null,'',url);}
function parameters(){const root=$('parameterControls');if(state.model==='mlp')root.innerHTML='<label>Disable neuron <input id="disable" type="number" min="-1" max="299" value="-1"> <small>−1 restores all</small></label>';else if(state.model==='cnn')root.innerHTML='<label>Negative activation slope <select id="slope"><option value="0">0 · trained ReLU</option><option value="0.01">0.01 · leaky ReLU</option><option value="0.1">0.1 · leaky ReLU</option><option value="0.3">0.3 · leaky ReLU</option></select></label>';else root.innerHTML='<label><input type="checkbox" id="positions" checked> Add learned position vectors</label>';
