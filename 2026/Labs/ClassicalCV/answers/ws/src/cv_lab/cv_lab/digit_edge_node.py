"""Digit recognition using Sobel, Canny, and Hough line evidence."""
...
class DigitEdgeNode(Node):
    """Extract richer edges from an upscaled 10x10 matrix, then match."""
    ...
    def __init__(self):
        ...
        self.report_publisher = self.create_publisher(
            String, '/perception/edge_digit_report', 10)
    ...
    def normalize_position(matrix):
        """Translate the foreground bounding box toward the 10x10 center."""
    ...
    def extract_edges(matrix):
        """Return image, Sobel magnitude, Canny edges, and Hough segments."""
        ...
        lines = cv2.HoughLinesP(
            canny, 1, np.pi / 180, threshold=22,
            minLineLength=22, maxLineGap=12)
        return image, magnitude, canny, lines
    ...
    def describe_edges(magnitude, canny, lines):
        """Turn the already-computed edge evidence into one descriptor."""
